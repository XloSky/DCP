// ============================================================
// DCP v3.0 — CONTEXT TAB
// All scoring and injection logic inline. No Library function calls.
// Scans recent history for active characters, scores scene categories,
// and injects the most relevant profile sections into context.
// ============================================================
const modifier = (text) => {
  ...
  // Returns true if 'word' appears in 'haystack' as a whole word
  // Prevents "art" matching inside "start", etc.
  ...
  // --- Find active characters (mentioned in recent history) ---
  ...
  // --- Build injection for each active character ---
  ...
  for (var i = 0; i < activeCharacters.length; i++) {
    ...
    // Fallback: if nothing scored, inject the fallback section
  ...
  if (injections.length > 0) {
    ...
    if (text.length + block.length < maxChars) {
      // Insert before the last line of context (typically the latest action)
